...
void dangnhap(SOCKET* hconnected, int& menuSwitch) {
	...
	int makeSure = 0;
	while (makeSure == 0) {
		client.Receive	((char*)&makeSure, sizeof(int), 0);
	}
	client.Receive((char*)&legit, sizeof(bool), 0);
	if (legit == 0) {
	...
	while (flag <= 3) {
		...
		if (sendAlarm == 1) {
			...
			if (flag == 1) {
				...
				client.Receive((char*)&checker, sizeof(bool), 0);
				if (checker == false) {
					flag++;
				}
				else {
...
void timkiem(SOCKET* hconnected)
{
	...
	if (sendAlarm == 1) {
		...
		for (int j = 0; j < 10; j++) {
			...
			int size = 0;
			int tempSize = 5;
			client.Receive((char*)&li[j].sameday, sizeof(int), 0);
			if (li[j].sameday == 0)
			{
	...
	cin.getline(nameRoom, 5);
	size = strlen(nameRoom);
	client.Send(&size, sizeof(size), 0);
	client.Send(&nameRoom[0], size, 0);
	client.Receive((char*)&flag, sizeof(flag), 0);
	cv::utils::logging::setLogLevel(utils::logging::LOG_LEVEL_SILENT);
	if (flag == 0) {
		cout << "Khong ton tai phong tren \n";
		system("pause");
		*hconnected = client.Detach();
		return;
	}
...
void huyPhong(SOCKET* hconnected) {
	CSocket client;
	client.Attach(*hconnected);
	int size;
	string billID;
	int flag = 0;
	cout << "Ma don hang ma khach hang muon xoa la gi ?\n";
	cin >> billID;
	size = billID.length();
	client.Send(&size, sizeof(size), 0);
	client.Send(&billID.c_str()[0], size, 0);
	client.Receive((char*)&flag, sizeof(int), 0);
	if (flag != 0) {
		client.Receive((char*)&flag, sizeof(int), 0);
		if (flag != 0) {
			cout << "Da huy thanh cong " << endl;
		}
		else {
			cout << "Xin loi quy khach bill quy khach da vuot qua han muc thoi gian cho phep \n";
		}
	}
	else {
		cout << "Xin loi quy khach bill tren khong ton tai " << endl;
	}
	(void)getchar();
	system("pause");
	*hconnected = client.Detach();
